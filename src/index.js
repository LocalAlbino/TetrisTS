...
function draw(ctx) {
    const spacing = 10, halfSpacing = 5, skyline = 190;
    for (let cell = skyline; cell < cells; cell++) {
        const row = calculateRow(cell), column = calculateColumn(cell);
        // row * spacing returns a value between [0, 390] to get the exact cell
        setColor(matrix[cell], ctx);
        if (cell < skyline + spacing) {
            ctx.fillRect(column * spacing, row * spacing - skyline, spacing, halfSpacing);
        }
        else {
            ctx.fillRect(column * spacing, row * spacing - skyline - halfSpacing, spacing, spacing);
        }
    }
    ctx.fill();
}
